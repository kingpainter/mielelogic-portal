...
class MieleLogicBookingCard extends HTMLElement {
  ...
  async _loadData() {
    try {
      ...
      this._errorCount = 0;
      // Load week availability lazily (non-blocking, doesn't count as error)
      this._loadWeek().catch(() => {});
  ...
  async _handleVaskehusChange(v) {
    ...
    this._weekDays = [];
    this._update();
    await Promise.all([this._loadSlots(), this._loadWeek()]);
  }
  ...
  _update() {
    ...
    // Booking er åben 24/7 — kun admin booking_locked spærrer knappen
    const btnDisabled = !canBook || this._loading || selectedIsBooked || locked;
      ...
      <div class="card-root">
        ...
        <div class="card-header">
          ...
          <div class="card-title-block">
            <div class="title-row"><span class="card-title">Vaskehus Booking</span>${!isOpen ? '<span class="pill pill-closed">· Lukket</span>' : ''}</div>
            <span class="card-sub">
        ...
        <div class="section">
          ...
          <div class="vhus-row">
            ...
            <button class="vhus-btn ${this._vaskehus === "Storvask" ? "vhus-on" : ""}" data-v="Storvask" ${this._loading ? "disabled" : ""}>Storvask</button>
          </div>
  ...
  _renderMachines(isOpen) {
    ...
    // Build lookup: machineNumber -> our booking
    const ourBookings = {};
    ...
    const SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" style="fill:none;stroke:var(--c);stroke-width:1.5;stroke-linecap:round;stroke-linejoin:round"><rect x="2" y="3" width="20" height="18" rx="2.5"/><circle cx="12" cy="13" r="5"/><circle cx="12" cy="13" r="2.2" style="stroke-width:1.2"/><line x1="6.5" y1="7.5" x2="9" y2="7.5" style="stroke-width:1.2"/><circle cx="5.5" cy="7.5" r="0.9" style="fill:var(--c);stroke:none"/><rect x="16" y="6" width="2.5" height="1.5" rx="0.4" style="fill:var(--c);stroke:none"/></svg>`;

    const items = this._machines.map(m => {
      const num   = String(m.MachineNumber ?? m.machine_number ?? m.id ?? "");
      const short = m.name.replace(/klatvask|storvask/gi, "").trim() || m.name;
      const bk    = ourBookings[num];
      const api   = m.state || "";

      let color, label;
      if (!isOpen) {
        color = "#2a2a2a"; label = "";
      } else if (bk && bk.active) {
        color = "#4ade80"; label = "I gang";
      } else if (bk) {
        color = "#60a5fa"; label = "Reserveret";
      } else {
        color = "#2a2a2a";
        label = (api === "running" || api === "reserved") ? "Optaget" : "";
      }

      const tip = bk
        ? (bk.active ? `${m.name}: Vores booking — i gang` : `${m.name}: Vores booking`)
        : `${m.name}: ${m.status || api}`;

      return `<div class="mach-item" title="${tip}"><div class="mach-ring" style="--c:${color}">${SVG}</div><div class="mach-name">${short}</div><div class="mach-state" style="color:${color}">${label}</div></div>`;
    }).join("");

    return `<div class="section"><div class="sec-label">MASKINER</div><div class="mach-row">${items}</div></div>`;
  }
  ...
  _bind() {
    ...
    root.querySelectorAll("[data-wdate]").forEach(btn => {
      btn.addEventListener("click", () => {
        this._selectedDate = btn.dataset.wdate;
        this._selectedSlot = "";
        this._loadSlots();
      });
    });
  ...
  _css() {
    return `
      ...
      .wd {
        ...
        flex: 1; min-width: 36px; padding: 6px 4px; border-radius: 8px;
        border: 1px solid #1e1e1e; background: #1a1a1a;
        cursor: pointer; font-family: inherit; transition: all 0.15s;
      }
      .wd:hover { border-color: #3a3a3a; background: #1e1e1e; }
      .wd-sel { border-color: #555 !important; background: #1e1e1e !important; }
      .wd-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
      .dot-free { background: #4ade80; }
      .dot-full { background: #ef4444; opacity: 0.55; }
      .wd-lbl { font-size: 9px; font-weight: 700; color: #333; text-transform: uppercase; letter-spacing: 0.05em; }
      .wd-lbl-today { color: #888; }
      .wd-today .wd-lbl { color: #888; }
      .wd-num { font-size: 12px; font-weight: 700; color: #666; }
      .wd-sel .wd-num { color: #e8e8e8; }
      /* Skeleton */
      .wd-skel { pointer-events: none; opacity: 0.4; }
      .wd-dot-skel { width: 7px; height: 7px; border-radius: 50%; background: #333; }
      .wd-lbl-skel { width: 20px; height: 8px; background: #2a2a2a; border-radius: 3px; }
      .wd-num-skel { width: 16px; height: 11px; background: #2a2a2a; border-radius: 3px; }
      /* ── SPINNER ─────────────────────────── */
