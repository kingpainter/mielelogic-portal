...
// Changes v2.2.0:
//   - Inline booking confirmation (replaces native confirm() dialog)
...
class MieleLogicPanel extends HTMLElement {
  constructor() {
    ...
    this._tab               = "booking";
    this._loading           = false;
    this._initialLoaded     = false;  // skeleton: true after first _loadCore completes
    this._error             = null;
    this._errCount          = 0;
    ...
    this._cleanupResult     = "";
    this._weekDays          = [];   // [{date,label,day_num,has_free,total,booked,is_today}]
  ...
  _onVisibility = () => {
    if (document.visibilityState === "visible" && !this._interval) {
      this._errCount = 0;
      this._startPolling();
      this._loadCore();
    }
  ...
  _startPolling() {
    ...
    this._interval = setInterval(() => {
      if (document.visibilityState === "visible") this._loadCore();
    }, 30000);
  }
  ...
  async _loadCore() {
    ...
    try {
      await Promise.all([this._loadSlots(), this._loadBookings(), this._loadStatus()]);
      this._errCount = 0;
      this._initialLoaded = true;
    } catch (e) {
      this._errCount++;
      this._initialLoaded = true; // show UI even on error, not stuck skeleton
      if (this._errCount >= 4) {
        this._stopPolling();
        const delay = Math.min(60000, 5000 * Math.pow(2, this._errCount - 4));
        this._backoffTimer = setTimeout(() => { this._errCount = 0; this._startPolling(); }, delay);
      }
    }
    this._render();
  }
  ...
  async _loadWeek() {
    if (!this._hass) return;
    this._weekLoading = true; this._render();
    try {
      const r = await this._hass.callWS({ type: "mielelogic/get_week_availability", vaskehus: this._vaskehus, start_date: new Date().toISOString().split("T")[0] });
      this._weekDays = r.days || [];
    } catch (e) { this._weekDays = []; }
    this._weekLoading = false; this._render();
  }

  async _loadHistory() {
    this._historyLoading = true; this._cleanupResult = ""; this._render();
    try { const r = await this._hass.callWS({ type: "mielelogic/get_history" }); this._history = r.history || []; this._historyLoaded = true; }
    catch (e) { this._history = []; this._historyLoaded = true; }
  ...
  // Called when user clicks ✕ on a booking row — shows inline confirmation
  _requestCancel(idx) {
    const b = this._bookings[idx];
    if (!b) return;
    this._confirm = { type: "cancel", idx, label: `${b.vaskehus || "Booking"} · ${this._fmtDate(b.Start)}` };
    this._render();
  }
  ...
  async _doCancel(idx) {
    ...
    try {
      ...
      if (r.success) { this._showToast("Booking slettet", "ok"); await new Promise(res => setTimeout(res, 400)); await this._loadCore(); }
      else { this._showToast(r.message, "warn"); }
    } catch (e) { this._showToast("Sletning fejlede: " + e.message, "err"); }
    this._loading = false; this._render();
  }

  async _doSaveDevices() {
    try { await this._hass.callWS({ type: "mielelogic/save_devices", devices: this._devices }); this._showToast("Enheder gemt", "ok"); }
    catch (e) { this._showToast("Kunne ikke gemme enheder", "err"); }
  }
  ...
  async _doSaveNotification(id, cfg) {
    try { await this._hass.callWS({ type: "mielelogic/save_notification", notification_id: id, config: cfg }); this._showToast("Notifikation gemt", "ok"); await this._loadNotifications(); this._render(); }
    catch (e) { this._showToast("Kunne ikke gemme", "err"); }
  }

  async _doTestNotification(id) {
    try { await this._hass.callWS({ type: "mielelogic/test_notification", notification_id: id }); this._showToast("Test besked sendt!", "ok"); }
    catch (e) { this._showToast("Test fejlede", "err"); }
  }
  ...
  async _doSaveEdit() {
    ...
    try {
      ...
      this._showToast("Skabelon gemt!", "ok"); this._closeModal();
    } catch (e) { this._showToast("Kunne ikke gemme", "err"); }
  }
  ...
  async _doResetNotification() {
    ...
    try {
      ...
      this._showToast("Nulstillet til standard!", "ok"); this._render();
    } catch (e) { this._showToast("Kunne ikke nulstille", "err"); }
  }
  ...
  async _doSaveAdmin() {
    ...
    try {
      ...
      this._showToast("Admin indstillinger gemt", "ok");
    } catch (e) { this._showToast("Kunne ikke gemme", "err"); }
  ...
  async _doCleanupHistory() {
    try {
      ...
      this._historyLoaded = false; await this._loadHistory();
  ...
  _showToast(msg, type = "ok") {
    ...
    this._renderToast();
    this._toastTimer = setTimeout(() => { this._toast = null; this._renderToast(); }, 3500);
  }

  _renderToast() {
    const root = this.shadowRoot;
    let el = root.querySelector(".toast-wrap");
    if (!this._toast) { if (el) el.remove(); return; }
    if (!el) { el = document.createElement("div"); el.className = "toast-wrap"; root.appendChild(el); }
    el.className = `toast-wrap toast-${this._toast.type}`;
    el.textContent = this._toast.msg;
  }

  _closeModal() { this._editingId = null; this._editTitle = ""; this._editMessage = ""; this._render(); }
  _cancelConfirm() { this._confirm = null; this._render(); }

  _fmtDate(ds) { return new Date(ds).toLocaleString("da-DK", { weekday: "short", day: "numeric", month: "numeric", hour: "2-digit", minute: "2-digit" }); }
  ...
  _html() {
    ...
    const confirmOverlay = this._confirm ? this._htmlConfirm() : "";
    return `<div class="panel-topbar">${this._htmlHeader(count, max)}${this._htmlTabs()}</div><div class="panel-scroll">${this._error ? `<div class="error-strip"><span>⚠ ${this._error}</span><button data-action="clear-error">✕</button></div>` : ""}${this._tab === "booking" ? (this._initialLoaded ? this._htmlBooking() : this._htmlSkeleton()) : ""}${this._tab === "notifications" ? this._htmlNotifications() : ""}${this._tab === "stats" ? this._htmlStats() : ""}${this._tab === "admin" ? this._htmlAdmin() : ""}${this._editingId ? this._htmlModal() : ""}${confirmOverlay}</div>`;
  }

  // Skeleton loading — shown on booking tab before first data arrives
  _htmlSkeleton() {
    const row = `<div class="skel-row"><div class="skel skel-icon"></div><div class="skel-lines"><div class="skel skel-line skel-l70"></div><div class="skel skel-line skel-l45"></div></div></div>`;
    return `<div class="card"><div class="skel skel-title"></div><div class="skel skel-select"></div><div class="skel skel-chips"></div><div class="skel skel-select"></div><div class="skel skel-btn"></div></div><div class="card"><div class="skel skel-title"></div>${row}${row}</div>`;
  }

  // Inline confirmation overlay — slides up from bottom of scroll area
  _htmlConfirm() {
    const c = this._confirm;
    if (c.type === "book") {
      return `<div class="confirm-overlay"><div class="confirm-box"><div class="confirm-icon">📋</div><div class="confirm-title">Bekræft booking</div><div class="confirm-detail">${this._esc(c.vaskehus)} · ${this._esc(c.date)}<br><strong>${this._esc(c.label)}</strong></div><div class="confirm-btns"><button class="confirm-cancel" data-action="cancel-confirm">Annuller</button><button class="confirm-ok" data-action="confirm-book">Book nu ✓</button></div></div></div>`;
    }
    if (c.type === "cancel") {
      return `<div class="confirm-overlay"><div class="confirm-box confirm-box-red"><div class="confirm-icon">🗑️</div><div class="confirm-title">Slet booking?</div><div class="confirm-detail">${this._esc(c.label)}</div><div class="confirm-btns"><button class="confirm-cancel" data-action="cancel-confirm">Behold</button><button class="confirm-ok confirm-ok-red" data-action="confirm-cancel-booking">Slet ✕</button></div></div></div>`;
    }
    return "";
  }
  ...
  _htmlBooking() {
    ...
    const bookBtn = this._loading ? `<button class="book-btn btn-loading" disabled><span class="spin">↻</span> Booker…</button>` : locked ? `<button class="book-btn btn-locked" disabled>🔒 ${s.lock_message || "Booking spærret"}</button>` : !canBook ? `<button class="book-btn btn-full" disabled>Max bookinger nået</button>` : `<button class="book-btn btn-ready" data-action="request-book">Book nu</button>`;
    const chips = this._slots.map(sl => {
      if (sl.booked) return `<span class="slot-chip chip-booked" title="Optaget">${sl.start} ✕</span>`;
  ...
  _htmlWeekStrip() {
    if (this._weekLoading) return `<div class="week-strip week-strip-loading">${[...Array(7)].map(()=>`<div class="wday wday-skel"><div class="skel skel-wlabel"></div><div class="skel skel-wdot"></div></div>`).join("")}</div>`;
    if (!this._weekDays.length) return "";
    return `<div class="week-strip">${this._weekDays.map(d => {
      const isSel = d.date === this._selectedDate;
      const cls = ["wday", isSel ? "wday-sel" : "", d.is_today ? "wday-today" : "", d.has_free ? "wday-free" : "wday-full"].filter(Boolean).join(" ");
  ...
  _bindEvents(root) {
    root.querySelectorAll("[data-tab]").forEach(b => b.addEventListener("click", () => {
      ...
      if (this._tab === "stats" && !this._historyLoaded) { this._loadHistory(); return; }
      if (this._tab === "notifications" && !this._notifLoaded) { this._loadNotifications().then(() => this._render()); return; }
      this._render();
    }));
    ...
    root.querySelectorAll("[data-chipslot]").forEach(chip => chip.addEventListener("click", () => {
      this._selectedSlot = chip.dataset.chipslot;
      const sel = root.querySelector("[data-field='slot']");
      if (sel) sel.value = this._selectedSlot;
      root.querySelectorAll("[data-chipslot]").forEach(c => c.classList.toggle("chip-selected", c.dataset.chipslot === this._selectedSlot));
    }));
    root.querySelectorAll("[data-action]").forEach(el => el.addEventListener("click", e => {
      const a = el.dataset.action;
      if (a === "request-book") this._requestBook();
      else if (a === "confirm-book") this._doBook();
      else if (a === "cancel-confirm") this._cancelConfirm();
      else if (a === "confirm-cancel-booking") this._doCancel(this._confirm?.idx);
      else if (a === "refresh") this._loadCore();
    ...
    // del-btn now triggers _requestCancel (inline confirm) instead of direct cancel
    root.querySelectorAll("[data-bindex]").forEach(b => b.addEventListener("click", () => { this._requestCancel(+b.dataset.bindex); }));
...
.panel-scroll{flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;padding:20px 28px 48px;position:relative}
...
.chip-selected{background:rgba(74,222,128,0.22)!important;border-color:#4ade80!important;box-shadow:0 0 0 2px rgba(74,222,128,0.25)}
.chip-booked{background:rgba(55,65,81,0.35);border:1.5px solid rgba(55,65,81,0.5);color:#6b7280;cursor:not-allowed;opacity:0.7;text-decoration:line-through}
...
.toast-wrap{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 20px;border-radius:10px;font-size:14px;font-weight:500;z-index:2000;pointer-events:none;animation:toast-in 0.2s ease;white-space:nowrap;box-shadow:0 4px 16px rgba(0,0,0,0.3)}
...
.skel{background:linear-gradient(90deg,var(--bg3) 25%,rgba(36,48,68,0.7) 50%,var(--bg3) 75%);background-size:200% 100%;animation:shimmer 1.4s infinite;border-radius:8px}
.skel-title{height:12px;width:120px;margin-bottom:14px}
.skel-select{height:42px;width:100%;border-radius:10px;margin-bottom:10px}
.skel-chips{height:24px;width:70%;margin-bottom:10px}
.skel-btn{height:46px;width:100%;border-radius:10px;margin-top:4px}
.skel-row{display:flex;align-items:center;gap:10px;padding:10px 0}
.skel-icon{width:4px;height:36px;border-radius:2px;flex-shrink:0}
.skel-lines{flex:1;display:flex;flex-direction:column;gap:6px}
.skel-line{height:12px;border-radius:6px}
.skel-l70{width:70%}
.skel-l45{width:45%}
@keyframes shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
.confirm-overlay{position:absolute;inset:0;background:rgba(15,25,35,0.75);backdrop-filter:blur(4px);z-index:500;display:flex;align-items:center;justify-content:center;padding:20px;animation:cfm-in 0.18s ease}
@keyframes cfm-in{from{opacity:0}to{opacity:1}}
.confirm-box{background:var(--bg2);border:1px solid var(--div);border-radius:var(--card-radius);padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.5)}
.confirm-box-red{border-color:rgba(239,68,68,0.3)}
.confirm-icon{font-size:32px;margin-bottom:12px}
.confirm-title{font-size:16px;font-weight:700;margin-bottom:8px}
.confirm-detail{font-size:13px;color:var(--sub);line-height:1.5;margin-bottom:20px}
.confirm-btns{display:flex;gap:10px}
.confirm-cancel{flex:1;padding:11px;background:var(--bg);border:1px solid var(--div);border-radius:10px;color:var(--sub);font-size:14px;font-weight:500;cursor:pointer;font-family:inherit;transition:all 0.2s}
.confirm-cancel:hover{border-color:var(--accent);color:var(--accent)}
.confirm-ok{flex:1;padding:11px;background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;border-radius:10px;color:#fff;font-size:14px;font-weight:600;cursor:pointer;font-family:inherit;transition:opacity 0.2s}
.confirm-ok:hover{opacity:0.9}
.confirm-ok-red{background:var(--red)}

.week-strip{display:flex;gap:5px;margin-bottom:10px;overflow-x:auto;scrollbar-width:none;padding:2px 0}
.week-strip::-webkit-scrollbar{display:none}
.week-strip-loading{pointer-events:none}
.wday{display:flex;flex-direction:column;align-items:center;gap:4px;padding:7px 6px;border-radius:10px;cursor:pointer;border:1.5px solid var(--div);background:var(--bg);flex:1;min-width:38px;transition:all 0.15s;font-family:inherit}
.wday:hover{border-color:var(--accent);background:var(--accent-glow)}
.wday-sel{border-color:var(--accent)!important;background:var(--accent-glow)!important}
.wday-today .wday-label{color:var(--accent);font-weight:700}
.wday-skel{pointer-events:none;border-color:transparent}
.wday-label{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;color:var(--sub)}
.wday-num{font-size:13px;font-weight:700;color:var(--text);font-family:'DM Mono',monospace}
