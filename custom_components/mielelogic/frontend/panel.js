...
// Changes v2.1.0:
//   - Split _loadAll: notifications only on tab switch (fewer WS calls)
//   - @import font removed from CSS (injected once via entrypoint.js)
//   - In-panel toast replaces persistent_notification for trivial feedback
//   - Slot chips are now clickable (sync with select)
//   - _historyLoaded flag replaces .length check (fixes empty-cache bug)
//   - _errCount: exponential backoff + auto-resume on visibilitychange
//   - data-cancel JSON → data-bindex (safer, no escaping issues)
//   - Cached state preserved on WS errors (no flicker to empty)
...
class MieleLogicPanel extends HTMLElement {
  constructor() {
    ...
    this._history           = [];
    this._historyLoaded     = false;   // fix: was .length check → stale empty cache
  ...
  _onVisibility = () => {
    // Resume polling and reload when tab becomes visible after error backoff
  ...
  async _init() {
    await Promise.all([this._loadCore(), this._loadAdmin()]);
  }

  // Primary load: slots + bookings + status only (fast, runs every 30s)
  async _loadCore() {
    ...
    try {
      ...
      this._errCount++;
      if (this._errCount >= 4) {
        // Exponential backoff: stop polling, retry after delay, resume on visibility
        this._stopPolling();
  ...
  // Notifications: only loaded on tab switch (slow, rarely changes)
  async _loadNotifLazy() {
    if (this._notifLoaded) return;
    await this._loadNotifications();
    this._notifLoaded = false; // allow reload on next explicit switch
  }
  ...
  async _loadSlots() {
    ...
    } catch (e) { this._error = "Kunne ikke hente tidslots"; /* keep previous slots */ }
  }
  ...
  async _loadBookings() {
    ...
    } catch (e) { /* keep previous bookings on error */ }
  }
  ...
  async _loadStatus() {
    try {
      ...
      this._status = r || {};
    } catch (e) { /* keep previous status on error */ }
  }
  ...
  async _doBook() {
    ...
    try {
      ...
      if (r.success) {
        this._showToast("✓ Booking oprettet!", "ok");
        await new Promise(res => setTimeout(res, 400));
        await this._loadCore();
      } else {
        this._error = r.message;
        this._showToast(r.message, "warn");
      }
  ...
  async _doCancel(idx) {
    ...
    try {
      ...
      if (r.success) {
        this._showToast("Booking slettet", "ok");
        await new Promise(res => setTimeout(res, 400));
        await this._loadCore();
      } else {
        this._showToast(r.message, "warn");
  ...
  async _doSaveNotification(id, cfg) {
    try {
      ...
      this._showToast("Notifikation gemt", "ok");
  ...
  // In-panel toast — replaces persistent_notification for non-critical feedback
  // type: "ok" | "warn" | "err"
  ...
  _render() {
    const root = this.shadowRoot;
    // Preserve toast across full renders
  ...
  _htmlHeader(count, max) {
    ...
    return `<div class="header"><div class="header-icon"><img src="${_ML_LOGO}" alt="MieleLogic"></div><div class="header-text"><h1>MieleLogic</h1><div class="header-meta">${count}${max ? ` / ${max}` : ""} booking${count !== 1 ? "er" : ""}${bal} · v2.1.0</div></div><button class="header-refresh" data-action="refresh" ${this._loading ? "disabled" : ""}><span class="${this._loading ? "spin" : ""}">↻</span> Opdater</button></div>`;
  }
  ...
  _htmlBooking() {
    ...
    // Chips: clickable for free slots, syncs with select
  ...
  _htmlBookingRow(b, idx) {
    ...
    // data-bindex instead of JSON in attribute — safer, no escaping issues
  ...
  _bindEvents(root) {
    ...
    // Clickable chips — sync with select and re-render chips highlight
    root.querySelectorAll("[data-chipslot]").forEach(chip => chip.addEventListener("click", () => {
      ...
      // Re-render chips only (lightweight)
    ...
    // data-bindex replaces JSON in data-cancel
    root.querySelectorAll("[data-bindex]").forEach(b => b.addEventListener("click", () => { this._doCancel(+b.dataset.bindex); }));
